...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(
        ...
        /*if (isLoginAttempt(request, response)) {
            try {
                ...
                return response403(request, response);
            }
        }*/
        ...
        //        try {
        //            if (JedisUtil.exists("TOKEN_" + authHeader)) {
        //                String user_str = JedisUtil.getJson("TOKEN_" + authHeader);
        //                if (StringUtils.isEmpty(user_str)) {
        //                    return response401(request, response);
        //                } else {
        //                    httpServletRequest.setAttribute("session", user_str);
        //                }
        //            } else {
        //                return response401(request, response);
        //            }
    ...
    /** 将非法请求跳转到 unauth */
    private boolean response401(ServletRequest req, ServletResponse res) {
        try {
            HttpServletResponse httpServletResponse = (HttpServletResponse) res;
            httpServletResponse.setStatus(401);
            httpServletResponse.setHeader("Content-Type", "application/json;charset=utf-8");
            PrintWriter writer = httpServletResponse.getWriter();
            writer.print("{\"code\":\"401\",\"message\":\"登录状态已失效\"}");
            writer.flush();
            writer.close();
        } catch (Exception e) {
